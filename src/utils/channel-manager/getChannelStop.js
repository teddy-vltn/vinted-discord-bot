...
import t from "../../t.js";
import { stopMonitoring } from "./stopMonitoring.js";

export async function getChannelStop(interaction) {
  try {
    ...
    const channelMenu = new StringSelectMenuBuilder()
      ...
      .setPlaceholder("Select the private channel to stop monitoring to.")
      .addOptions(
    ...
    if (!interaction.replied) {
      await interaction.reply({
        content:
          "Please select the private channel you want to stop monitoring to:",
        components: [row],
        ephemeral: true,
      });
    } else {
    ...
    channelCollector.on("collect", async (channelInteraction) => {
      ...
      stopMonitoring(interaction, channelId);
    });
  } catch (error) {
