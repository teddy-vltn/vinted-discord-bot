...
export async function createCategoryIfNotExists(channelManager, categoryName) {
  let category = channelManager.cache.find(
...
export async function createPrivateThread(
  ...
  const channel = await category.guild.channels.fetch(currentChannelId);
    ...
    for (const channel of channels) {
      ...
      if (hoursAgo(lastUpdated) > discordConfig.channel_inactivity_hours) {
        ...
        const member = await client.guilds.cache
          .get(guild_id)
          .members.fetch(userData.discordId)
          .catch(() => null);
        ...
        const embed = await createBaseEmbed(
          ...
          t(user.locale, "reply-to-keep-channel", {
        ...
        collector.on("collect", async (interaction) => {
          if (interaction.customId === "keep_channel" + userData.discordId) {
            ...
            await interaction.reply({
              content: t(user.locale, "channel-kept"),
              ephemeral: true,
            });
          } else if (
            interaction.customId ===
            "delete_channel" + userData.discordId
          ) {
            // User wants to delete the channel
            await crud.deleteVintedChannelByChannelId(channelId);

            await interaction.reply({
              content: t(user.locale, "channel-deleted"),
              ephemeral: true,
            });

            // Delete the channel if it still exists
            const discordChannel = client.guilds.cache
              .get(guild_id)
