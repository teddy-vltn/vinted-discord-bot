...
function buildCategoryMap(node, parentMap = {}) {
  ...
  if (node.catalogs && Array.isArray(node.catalogs)) {
    node.catalogs.forEach((child) => {
      ...
      parentMap[parentId] = parentMap[parentId].concat(
        parentMap[childId] || []
      );
    });
  }
...
const userSchema = new Schema({
  ...
  maxChannels: {
    type: Number,
    default: ConfigurationManager.getUserConfig.max_private_channels_default,
  },
