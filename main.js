...
import {
  createVintedItemActionRow,
  createVintedItemEmbed,
} from "./src/bot/components/item_embed.js";
...
import {
  checkVintedChannelInactivity,
  postMessageToChannel,
} from "./src/services/discord_service.js";
...
const sendToChannel = async (item, user, vintedChannel) => {
  ...
  const doMentionUser =
    user && vintedChannel.preferences.get(Preference.Mention);
...
const monitorChannels = () => {
  const handleItem = async (rawItem) => {
    ...
    if (
      item.getNumericStars() === 0 &&
      algorithmSettings.filter_zero_stars_profiles
    ) {
      return;
    }
